...
public class GameView extends View implements AreaCallback {
	...
	*/
	...
	public int getTextSize(){
		...
		return Math.min(mXScreenSize, mYScreenSize) / 50;
	}
	...
	protected void updateAreaInfo(int w, int h, int oldw, int oldh){
		...
		if(isScreenLandScape()){
			if(mBlockNumOnLongLine < 0){
				xBlockCount = mXScreenSize / blockSize - 3; //保证有地方显示 waitingArea
			...
			if(mBlockNumOnLongLine < 0){
				yBlockCount = mYScreenSize / blockSize - 3; //保证有地方显示 waitingArea
	...
	public void nextState(int newState) {
		...
		if(inRunningState()) {
			...
			if( ! isRunningState(oldState)) {
				Toast.makeText(getContext(), R.string.tip_menu, Toast.LENGTH_SHORT).show();
			}
		}
		else{
