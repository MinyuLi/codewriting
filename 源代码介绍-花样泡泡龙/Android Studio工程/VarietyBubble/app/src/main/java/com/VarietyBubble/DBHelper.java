...
public class DBHelper extends SQLiteOpenHelper {
	...
	public long saveBlock(int x, int y, int color, int shape, int ascription){
		try{
			...
			int value = Global.Make4BytesToInt((byte)x, (byte)y, (byte)blockInfo,
					(byte)ascription);
	...
	public long saveBlock(int dbItem){
		try{
			SQLiteDatabase db = getWritableDatabase();

			ContentValues values = new ContentValues();
			values.put(TBlocks.block, dbItem);
			long id = db.insert(TBlocks.tbName, TBlocks.key, values);
			//Log.i("DBHelper:saveBlock", String.format("id=%d, value=0x%x", id, value));
			return id;
		}catch(SQLiteException e){
	...
	public long saveRecord(Util.Record r, Crypto crypto){
		try{
			SQLiteDatabase db = getWritableDatabase();
	...
	public ArrayList<Util.Record> loadRecords(Crypto crypto){
		ArrayList<Util.Record> records = new ArrayList<Util.Record>();
		try{
			SQLiteDatabase db = getReadableDatabase();
