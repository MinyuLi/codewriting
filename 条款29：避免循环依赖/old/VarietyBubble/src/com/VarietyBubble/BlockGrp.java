...
public class BlockGrp {
	...
	public BlockGrp(GameArea father){
		mFather = father;
	}
	...
	public boolean revolve(int direction){
		...
		for(Block d : mBlockList){			
			...
			if( ! mFather.isPositionEmpty(pt.x, pt.y))
				return false; //不能转
	...
	public int downOneStep(){
		...
		for(int y = maxY; y >=0; y--){
			for(Block b : mBlockList){
				...
				if( ! mFather.isPositionEmpty(b.getX(), b.getY()+1)){
					mFather.addBlock(b);
					mBlockList.remove(b);
					y++;
	...
	public boolean move(int xOffset, int yOffset){
		...
		for(Block b : mBlockList){
			if( ! mFather.isPositionEmpty(b.getX()+xOffset, b.getY()+yOffset))
				return false;
		}
		for(Block b : mBlockList){
