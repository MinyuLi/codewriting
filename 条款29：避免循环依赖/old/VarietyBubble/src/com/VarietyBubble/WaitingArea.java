...
public class WaitingArea {
	...
	public WaitingArea(GameView view, int blockGrpNum, Rect rect){
		mFather = view;
	...
	public void draw(Canvas canvas, Paint paint){
		...
		if(mFather.isScreenLandScape()){
			for(BlockGrp dg : mBlockGrpList){
				dg.drawSmallSize(canvas, paint, xOffset, yOffset - mRollingOffset);
				yOffset += (blockSize*3+mYOffset);
			}
		}else{
			xOffset = mRect.right - mXOffset - blockSize * 3;
			for(BlockGrp dg : mBlockGrpList){
				dg.drawSmallSize(canvas, paint, xOffset + mRollingOffset, yOffset);
	...
	public void save(DBHelper db){
		...
		int ascription = DBHelper.BlockAscription.inBlockGrpList_0;
	...
	public boolean addBlock(Block b, int ascription){
		int pos = ascription - DBHelper.BlockAscription.inBlockGrpList_0;
