	...
	*/
	...
	protected void updateAreaInfo(int w, int h, int oldw, int oldh){
		...
		if(mGameArea == null || mWaitingArea == null){
			...
			mWaitingArea = new WaitingArea(this, blockGrpNum, waitingAreaRect);
		}
		else if(w == oldh && h == oldw){
			//屏幕旋转
			mGameArea.revolveScreen();
			mWaitingArea.setBlockGrpNum(blockGrpNum);
			mWaitingArea.setRect(waitingAreaRect);
	...
	private void drawNextBlocks(Canvas canvas, Paint paint){
		...
		if(mWaitingArea != null)
			mWaitingArea.draw(canvas, paint);
	}
	...
	public boolean loadRecords(SharedPreferences sp, DBHelper db, Crypto crypto){
		mRecords = db.loadRecords(crypto);
	...
	protected int loadBlocks(DBHelper db){
		...
		for(int v : blocks){
			x = v & 0x000000ff;
			y = (v & 0x0000ff00) >> 8;
			color = (v & 0x000f0000) >> 16;
			shape = (v & 0x00f00000) >> 20;
			ascription = (v & 0xff000000) >> 24;
			
			Block b = Block.Create(x, y, color, shape);
			if(b == null)
				continue;
			if(ascription == DBHelper.BlockAscription.inGameArea){
				if(mGameArea != null){
					mGameArea.addBlock(b);
				}
			}else if(ascription == DBHelper.BlockAscription.inDroppingBlockGrp){
				mGameArea.addBlockToBlockGrp(b);
			}else if(ascription == DBHelper.BlockAscription.inBackupBlockGrp){
				addBlockToBackupBlockGrp(b);
			}else if(ascription >= DBHelper.BlockAscription.inBlockGrpList_0){
	...
	public int saveGame(SharedPreferences sp, DBHelper db, Crypto crypto){		
		...
		mGameArea.save(db);
		if(mBackupBlockGrp != null){
			mBackupBlockGrp.save(db, DBHelper.BlockAscription.inBackupBlockGrp);
		}
